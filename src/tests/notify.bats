...
@test "16: SanitizeVars - Variable with pre-existing spaces should not get duplicated" {
  # Test that variables with existing 2 spaces + newline don't get extra spaces
  COMMIT_MESSAGE="$(printf "%s  \n%s" "First line" "Second line")" # Already has 2 spaces + newline
  EXPECTED="First line  \\nSecond line"                            # Should not become 4 spaces
  template='Commit: ${COMMIT_MESSAGE}'

  SanitizeVars "$template"

  printf '%s\n' "Expected: '$EXPECTED'" "Actual: '$COMMIT_MESSAGE'"
  [ "$COMMIT_MESSAGE" = "$EXPECTED" ] # Should not duplicate spaces
}

@test "17: SanitizeVars - Multi-line variable should get proper CommonMark line breaks" {
  # Test that multi-line variables are formatted with proper CommonMark line breaks
  # Each line (except the last) should end with 2 spaces + \n
  ...
  printf '%s\n' "Expected: '$EXPECTED'" "Actual: '$COMMIT_MESSAGE'"
  [ "$COMMIT_MESSAGE" = "$EXPECTED" ] # Multi-line should have 2 spaces + \n between lines
}

@test "18: Template processing maintains CommonMark formatting throughout pipeline" {
  CIRCLE_JOB="test-job"
  CIRCLE_PROJECT_REPONAME="test-repo"
  GLUE_DEFAULT_TARGET="general"

  template='**Job Succeeded!**  
**Job:** ${CIRCLE_JOB}  
**Project:** $CIRCLE_PROJECT_REPONAME'

  # Simulate the template processing flow
  template_body="$(eval printf '%s' \""$template\"")"
  json_body=$(jq -n --arg text "$template_body" '{text: $text}')
  SanitizeVars "$json_body"

  # Extract the final text from JSON
  final_text=$(echo "$json_body" | jq -r '.text')

  # Check that CommonMark formatting is preserved
  printf '%s\n' "Final text: '$final_text'"

  # Each line break should be properly formatted
  [[ "$final_text" =~ "**Job Succeeded!**  " ]] # First line should end with 2 spaces + newline
  [[ "$final_text" =~ "**Job:** test-job  " ]]  # Second line should end with 2 spaces + newline
}
