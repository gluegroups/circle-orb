...
BuildMessageBody() {
  ...
  if [ -n "${GLUE_PARAM_CUSTOM:-}" ]; then
    ...
    if [ -n "${GLUE_PARAM_TEMPLATE:-}" ]; then
      TEMPLATE="\$$GLUE_PARAM_TEMPLATE"
...
InstallJq() {
  ...
  if command -v curl >/dev/null 2>&1 && ! command -v jq >/dev/null 2>&1; then
    ...
    command -v curl >/dev/null 2>&1 || {
      echo >&2 "GLUE ORB ERROR: CURL is required. Please install."
      exit 1
    }
    command -v jq >/dev/null 2>&1 || {
      echo >&2 "GLUE ORB ERROR: JQ is required. Please install"
      exit 1
    }
    return $?
  fi
...
FilterBy() {
  ...
  if { [ "$FLAG_MATCHES_FILTER" = "false" ] && [ "$GLUE_PARAM_INVERT_MATCH" -eq 0 ]; } ||
    { [ "$FLAG_MATCHES_FILTER" = "true" ] && [ "$GLUE_PARAM_INVERT_MATCH" -eq 1 ]; }; then
    # don't send message.
    echo "NO GLUE ALERT"
    echo
...
SanitizeVars() {
  ...
  for var in $variable_names; do
    ...
    # Escape newlines for CommonMark compliance.
    # Each line break should have exactly 2 spaces before \n
    sanitized_value="$(printf '%s' "$sanitized_value" | awk '
      {
      ...
      END {
        for (i = 1; i <= NR; i++) {
          if (i == 1) {
            ...
            # Check if previous line already ends with 2 spaces
            if (lines[i-1] ~ /  $/) {
              # Previous line already ends with 2 spaces, just add \n
              printf("\\n%s", lines[i])
            } else if (lines[i-1] ~ / $/) {
              # Previous line has 1 space, add 1 more space + \n
              printf(" \\n%s", lines[i])
            } else {
              # Previous line has no trailing spaces, add 2 spaces + \n
              printf("  \\n%s", lines[i])
            }
          }
        }
